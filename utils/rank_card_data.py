...
async def get_rank_card_data(guild_id: int, user_id: int,
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        # Catalog size for the card's "owned / total" inventory header
        # (the reference shows e.g. "12 / 48"); read-only COUNT, same DB
        # round-trip as the rest of the aggregation.
        catalog_cursor = await db.execute(
            "SELECT COUNT(*) FROM item_catalog WHERE guild_id=?", (guild_id,))
        inventory_total = (await catalog_cursor.fetchone())[0]
    ...
    # Booster-only Prestige VI: effective_prestige == BOOSTER_TIER only
    # when the member is actively boosting AND activated the VI shop
    # entry (utils/prestige.get_effective_prestige). is_booster alone no
    # longer implies VI.
